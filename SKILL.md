...
4. **Draw one colorful SVG to a world-class bar.** Build it for the `visualize` tool's `show_widget` (SVG mode). **[QUALITY.md](QUALITY.md) is the standard — read it before drawing and run its pre-flight checklist before you finalize.** The essentials it covers:
   ...
   - **Before rendering, run the text-width and overlap checks from QUALITY.md §8: resolve every element's absolute bounding box and check every pair for intersection.** This is what catches text spilling past a card edge, an icon landing on top of a connector line, or a label crossing another box — compute it, don't eyeball it. Do not call `show_widget` until it passes.
